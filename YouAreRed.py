...
class Game:
    def __init__(self, gameState):
        ...
        self.message = '253,112,True,none'
    ...
    def run(self, ipANDport, port):
        ...
        while self.gameStateRun:
            ...
            self.player2.pos = [float(self.VARIABLES[0]), float(self.VARIABLES[1])]
            if self.VARIABLES[2] == 'True':
                self.player2.flip = True
            else:
                self.player2.flip = False

            # Assign these values to the player's attributes
    ...
    def sending(self):
        time.sleep(0.1)
        while True: # sending
            message = str(self.player.pos[0]) + ',' + str(self.player.pos[1]) + ',' + str(self.player.flip) + ',' + str(self.gameLogic.move0)
